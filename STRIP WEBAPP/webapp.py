...
def get_history(limit):
    ...
    list_of_results = create_list(records)

    cursor.close()
    return jsonify(results = list_of_results), 201
...
def addEntry(user_name, user_result):
    ...
    val = (user_name, result['name'][0].lower(), result['accuracy'], result['image'], user_result[0].lower(), object_detector.GET_MODEL_VERSION())
...
def process_form():
    ...
    sql = "INSERT INTO strip (userId,result,accuracy,photo,userResult, modelVersion) VALUES (%s, %s, %s ,%s ,%s, %s)"       # sql query
    val = (session['userName'], result['name'][0].lower(), result['accuracy'], result['image'], session['userResult'][0].lower(), object_detector.GET_MODEL_VERSION())
